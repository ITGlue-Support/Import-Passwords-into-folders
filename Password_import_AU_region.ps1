...
try {
    ...
    $gen_jwt_url = "https://$($subdomain).au.itglue.com/login?generate_jwt=1"
    $gen_jwt = Invoke-RestMethod -Uri $gen_jwt_url -Method 'POST' -Body $body -Headers $auth_headers

    $gen_access_token_url = "https://$($subdomain).au.itglue.com/jwt/token?refresh_token=$($gen_jwt.token)"
...
function get_org_id {
    ...
    try {
        $org_url = "https://api.au.itglue.com/organizations?filter[name]=$org_name"
...
function get_pass_folder {
    ...
    try {
        $pass_folder_URL = "https://api.au.itglue.com/organizations/$id/relationships/password_folders"
...
"@
        $create_new_category = Invoke-RestMethod 'https://api.au.itglue.com/password_categories' -Method 'POST' -Headers $headers -Body $type_body
...
function get_pass_type {
    ...
    try {
        $type_url = "https://api.au.itglue.com/password_categories?filter[name]=$type_name"
...
"@
    ...
    try {
        $new_pass = Invoke-RestMethod "https://api.au.itglue.com/passwords" -Method 'POST' -Headers $headers -Body $pass_create_body
